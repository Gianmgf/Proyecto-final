...
Una vez estabilizadas la sincronización y la ecualización, el receptor puede decodificar la señal TMCC con suficiente confiabilidad como para configurar adecuadamente el resto de la cadena. El TMCC contiene todos los parámetros relevantes de la transmisión, con excepción del modo y del intervalo de guarda. Para demodularlo, el receptor debe identificar la secuencia de referencia en el primer símbolo del cuadro y luego aplicar la modulación diferencial DBPSK inversa sobre los símbolos siguientes, aplicando una votación sobre las subportadoras TMCC para determinar si se invirtió la secuencia del símbolo anterior o no y luego aplicar la decodificación del código DSC(184,102) para corregir errores y recuperar los bits de información.


Los campos TMCC permiten determinar, para cada capa jerárquica activa, la modulación utilizada, la tasa del código convolucional, la profundidad del entrelazado temporal y el número de segmentos asignados. También permiten identificar si el segmento 0 está reservado para recepción parcial. En consecuencia, el TMCC fija la configuración de la cadena de recepción.

\section{Procesamiento del cuadro OFDM y separación de capas}

Una vez obtenidas las subportadoras ecualizadas, el receptor invierte las etapas aplicadas durante la conformación del cuadro OFDM. En primer lugar, separa las portadoras de datos de aquellas reservadas para pilotos, TMCC y canales auxiliares, conservando únicamente los símbolos complejos asociados a la información útil. En la implementación descrita en \cite{uruguay_rx}, el mismo bloque encargado de procesar el TMCC también filtra las portadoras de control y entrega a la salida únicamente las portadoras de datos, ya ordenadas por número de segmento.

A continuación, se invierte el entrelazado en frecuencia aplicado en transmisión. Este proceso se realiza en el orden inverso al del transmisor: primero se deshace la aleatorización intra-segmento, luego la rotación de portadoras dentro del segmento y, finalmente, el entrelazado entre segmentos. De esta manera, los símbolos recuperan la posición espectral que tenían antes de la etapa de intercalado frecuencial.

Una vez restaurado ese orden, el receptor puede reagrupar los datos según la capa jerárquica a la que pertenecen. En configuraciones típicas, esto permite separar el segmento central asociado a recepción parcial de los restantes segmentos destinados a recepción total. En este sentido, un receptor \textit{full-seg} procesa los 13 segmentos del sistema, mientras que un receptor \textit{1-seg} únicamente demodula el segmento central \cite{isdbt,portugues,uruguay_rx}.

\section{Demodulación y decodificación de canal}

Luego de la separación por capas, el receptor aplica el demodulador correspondiente a la modulación indicada por el TMCC. Después del demapeo, se invierte el entrelazado de bits. Este bloque es el inverso del entrelazador del transmisor, por lo que puede interpretarse como la misma estructura de retardos pero recorrida en sentido contrario. De esta manera, los bits recuperan el orden con el que salían originalmente del codificador convolucional.

A continuación, se invierte el entrelazado temporal. Del mismo modo que en el caso anterior, el desentrelazador puede verse como el mismo banco de retardos del transmisor, pero aplicado en orden inverso. Esto permite restituir la secuencia temporal original de símbolos antes de la decodificación interna.

Una vez restaurado el orden temporal, el receptor aplica la decodificación convolucional correspondiente a la tasa configurada en transmisión. Si la tasa efectiva se obtuvo mediante punzado, el receptor debe reconstruir primero la secuencia equivalente al código madre de tasa \(1/2\) y luego aplicar el algoritmo de Viterbi para estimar la secuencia de bits más probable \cite{isdbt,portugues}. En la implementación descrita en \cite{uruguay_rx}, el decodificador convolucional reutiliza bloques equivalentes a los empleados en receptores DVB-T.

Luego de la decodificación interna, el flujo binario se vuelve a agrupar en bytes y se aplica el desentrelazado de byte, nuevamente como el bloque inverso del entrelazador convolucional de bytes del transmisor. Tras ello, se realiza la desdispersión de energía utilizando la misma secuencia PRBS y la misma inicialización empleadas en transmisión. Como la operación XOR es involutiva, basta con volver a aplicar la misma secuencia pseudoaleatoria para recuperar el flujo original, preservando el byte de sincronismo del transport stream \cite{isdbt,portugues}.
...
Al finalizar la decodificación Reed--Solomon, el receptor obtiene uno o más flujos MPEG-2 TS, según la cantidad de capas jerárquicas activas en la transmisión. En una configuración de una sola capa, el resultado es un único \textit{transport stream}. En configuraciones jerárquicas, en cambio, pueden recuperarse varios flujos independientes, cada uno asociado a una capa y a su correspondiente grupo de segmentos.

En conjunto, la cadena receptora puede entenderse como la inversión ordenada de la cadena transmisora: primero se desarma el cuadro OFDM y se recuperan las capas, luego se invierten los entrelazados, la modulación y la codificación de canal, y finalmente se reconstruye el flujo MPEG-2 TS. De este modo, junto con el Capítulo~\ref{chap:isdbtb_tx}, queda descrita una visión completa del transceptor ISDB-Tb.
