...
La adquisición inicial del símbolo OFDM aprovecha la redundancia introducida por el prefijo cíclico. Si $N$ es la longitud útil del símbolo y $L$ la longitud del prefijo cíclico, los últimos $L$ samples del símbolo coinciden con sus primeros $L$ samples. Esta propiedad permite construir una métrica de autocorrelación sobre la señal recibida $r[k]$ y estimar el comienzo del símbolo mediante
...
Ya una vez decodificada la señal TMCC, el siguiente paso es aislar las subportadoras de datos y volver a construir los segmentos de datos. Para ello simplemente se deben filtrar todas las subportadoras que no correspondan a datos, es decir, aquellas reservadas para pilotos, TMCC y canales auxiliares.
